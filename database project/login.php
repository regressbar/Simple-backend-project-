...
while($row=mysqli_fetch_assoc($query))
 {
  ...
  if($username == $dbusername && $password == $dbpassword && $is_admin == 1)
 {
	// старое место расположения
	//  session_start();
	 $_SESSION['session_username']=$username;	 
 /* Перенаправление браузера */
   header("Location: Admin/Site/homeAdmin.html");
	} else if($username == $dbusername && $password == $dbpassword && $is_admin == 0)
	{
	   // старое место расположения
	   //  session_start();
		$_SESSION['session_username']=$username;	 
